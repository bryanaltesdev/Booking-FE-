...
function Create(props) {
  ...
  const editReservation = () => {
    ...
    const data = {
      note: note,
      startDate: new Date(startDate).getTime()/1000 + t1,
      endDate: new Date(endDate).getTime()/1000 + t2,
      resource: resource
    };
    axios.defaults.headers.post['Content-Type'] = 'application/json;charset=utf-8';
    axios.defaults.headers.post['Access-Control-Allow-Origin'] = '*';
    axios.put(APIURL + `reservation/` + props.id, data)
    .then(res => {
  ...
  useEffect(() => {
    ...
    axios.get(APIURL+`resource`)
      .then(res => {
        ...
        document.getElementById(props.data.resource._path.segments[1]).click();
      })
    setNote(props.data.note);
    console.log(props.data.startDate.toString().slice(16, 21));
    setStartTime(props.data.startDate.toString().slice(16, 21));
    setStartDate(props.data.startDate.toString().slice(0, 15));
    setEndDate(props.data.endDate.toString().slice(0, 15));
    setEndTime(props.data.endDate.toString().slice(16, 21));
  }, []);
  return (
    <div>
      {
        location.pathname === '/Reservation/Edit' ?
          <form>
            <div className="shadow overflow-hidden sm:rounded-md my-10">
              <div className="px-4 py-5 bg-white sm:p-6">
                <div className="grid grid-cols-6 gap-6">
                  <div className="col-span-6 sm:col-span-2">
                    ...
                    <input type="text" defaultValue={props.data.note} onChange={(e) => { setNote(e.target.value) }} name="calendar-name" id="first-name" autoComplete="given-name" className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                  </div>
                </div>
              </div>
              <div className="px-4 py-5 bg-white sm:p-6">
              ...
              <div className="px-4 py-3 bg-gray-50 text-right sm:px-6 mt-15">
                <button type="button" onClick={editReservation} className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                  Save
                </button>
              </div>
            </div>
